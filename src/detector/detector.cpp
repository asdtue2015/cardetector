...
//     cpu_hog.setSVMDetector(detector);
	...
	while (running) {
		...
		while (running && !frame.empty()) // as long as running is set to be true and we still have frames to run then
		{
			...
			// values essential for clusters drawing

			float h; //height of cluster
			float w; // widht of cluster
			float SI; // Intersected area between two clusters
			float overlap; //overlap (0-1) between two clusters (SI/min(Cluster_area))

			//Initializing grouping

			int Clusters; // number of initial clusters needed as input in kmeans function
			int clusterCount; // number of final clusters calculated by for individual cases
			int attempts = 1000; // number of iterations before convergence
			Mat labels(found.size(), 1, CV_32F);
			Mat centers;

			// assigning value for initial clusters based on the amoun of detections from HOG Descriptor		

			if (found.size() >= 20) //for too many detections (>=20)
					{
				Clusters = 20; //set a reasonable value to start with respect to time consuption

			} else {
				Clusters = found.size(); //for less detections (<20)
			}

			// data needed for the calculating the final number of clusters 

			Mat mnarea(Clusters, 1, CV_32F); // contains the mean area of each cluster with respect to the included detections
			Mat A(Clusters, 2, CV_32F); // contains the coordinates of Top-Left point of each cluster - needed for the clusters intersection
			Mat B(Clusters, 2, CV_32F); // contains the coordinates of Right-Bottom point of each cluster - needed for the clusters intersection

			// Calculating the final number of clusters 
			// Architecture.. Start with high number of clusters and slightly decrease them as long as the calculated clusters does not intersect among them	

			for (clusterCount = Clusters; clusterCount > 0; clusterCount--) {

				// call buit-in kmeans function for the clustering
				kmeans(samples, clusterCount, labels,
						TermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 1000,
								0.0001), attempts, KMEANS_PP_CENTERS,
						centers);

				//calculate mean area of each cluster based on the detection it contains
				for (int k = 0; k < clusterCount; k++) {    // for every cluster
					...
					w = (mnarea.at<float>(0, k)) / h; // calculate the wigth of each calculated cluster

					A.at<float>(k, 0) = centers.at<float>(k, 0) - w / 2; //top left point of cluster - x 
					A.at<float>(k, 1) = centers.at<float>(k, 1) + h / 2; //top left point of cluster - y

					B.at<float>(k, 0) = centers.at<float>(k, 0) + w / 2; //bottom right point of cluster - x
					B.at<float>(k, 1) = centers.at<float>(k, 1) - h / 2; //bottom right point of cluster - y

				}

				// condition that assures that we will end with at least one final cluster
				...
				for (int j = 0; j < clusterCount; j++)  // for every cluster
						{
					for (int lenght = 1; lenght < clusterCount; lenght++) // for all the other clusters
							{
						// calculate the ntersected area
						SI = fmax(0,
								fmin(B.at<float>(j, 0),
										B.at<float>(j + lenght, 0))
										- fmax(A.at<float>(j, 0),
												A.at<float>(j + lenght, 0)))
								* fmax(0,
										fmin(A.at<float>(j, 1),
												A.at<float>(j + lenght, 1))
												- fmax(B.at<float>(j, 1),
														B.at<float>(j + lenght,
																1)));
						// calculate the overlap
						overlap = SI
								/ (fmin(mnarea.at<float>(0, j),
										mnarea.at<float>(0, j + lenght)));

						if (overlap >= 0.5) //if condition of merging further is satisfactory

							break;    // check is done

					}

					if (overlap >= 0.5) // again to break the next loop

						break;  // check is done
				}

				if (overlap < 0.5) // if all previous check has failed then we cannot reduce further the number of final clusters

						{
					break; // calculations are done ... final number of clusters is calculated
				}

			}

			// check the difference between initial and final number of clusters
			cout << "Initial Number of Clusters" << " " << Clusters << endl;
			cout << "Final Number of Clusters" << " " << clusterCount << endl;

			for (int i = 0; i < clusterCount; i++) // for the final number of clusters
					{

				h = sqrt( // calculate the height of each calculated cluster         
						(mnarea.at<float>(0, i))
								* ((float) height / (float) width));
				w = (mnarea.at<float>(0, i)) / h; // calculate the width of each calculated cluster

				rectangle(
						img_to_show,               // draw the clusters
						Point(centers.at<float>(i, 0) - w / 2,
								centers.at<float>(i, 1) - h / 2),
						Point(centers.at<float>(i, 0) + w / 2,
								centers.at<float>(i, 1) + h / 2),
						CV_RGB(255, 0, 0), 3);
			...
			if (args.src_is_directory)
				break; // if processing a folder, get the next image instead of looping
