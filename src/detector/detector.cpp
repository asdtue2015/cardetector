    ...
    while (running) {
        ...
        while (running && !frame.empty()) // as long as running is set to be true and we still have frames to run then
        {
            ...
            /* based on the produced output alternation when the mode is toggled between cpu and gpu although the thresholds didn change so it could
             be inferred from the alternation in the output that detectmulti scale works differently depending on whether its from cpu_hog or gpu_hog */
            ...
            kmeans(samples, clusterCount, labels,
                    TermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 10000,
            ...
            for (int i = 0; i < clusterCount; i++) {
                ...
                rectangle(img_to_show,
                        Point((int) centers.at<float>(i, 0) - w / 2,
                                (int) centers.at<float>(i, 1) - h / 2),
                        Point((int) centers.at<float>(i, 0) + w / 2,
                                (int) centers.at<float>(i, 1) + h / 2),
                        CV_RGB(0, 0, 255), 3);
